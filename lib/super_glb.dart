...
void rotateGlb(GlbFile glb, double degX, double degY, double degZ) {
  ...
  final Set<int> rotatedAddresses = {};
  final Set<int> targetAccessorIndices = {};
  ...
  for (var mesh in (glb.json["meshes"] ?? [])) {
    for (var prim in (mesh["primitives"] ?? [])) {
      ...
      if (attr.containsKey("POSITION")) {
        targetAccessorIndices.add(attr["POSITION"]);
      }
      if (attr.containsKey("NORMAL")) {
        targetAccessorIndices.add(attr["NORMAL"]);
      }
    }
  }

  for (int accIdx in targetAccessorIndices) {
    final acc = accessors[accIdx];
    if (acc["type"] != "VEC3" || acc["componentType"] != 5126) {
      continue;
    }
    
    final bv = views[acc["bufferView"]];
    final baseOffset = (bv["byteOffset"] ?? 0) + (acc["byteOffset"] ?? 0);
    ...
    for (int i = 0; i < count; i++) {
      final p = baseOffset + i * stride;
      
      if (rotatedAddresses.contains(p)) {
        continue;
      }

      final x = bd.getFloat32(p, Endian.little);
      final y = bd.getFloat32(p + 4, Endian.little);
      final z = bd.getFloat32(p + 8, Endian.little);
      
      final vec = apply(m, x, y, z);

      bd.setFloat32(p, vec[0], Endian.little);
      bd.setFloat32(p + 4, vec[1], Endian.little);
      bd.setFloat32(p + 8, vec[2], Endian.little);

      rotatedAddresses.add(p);
    ...
    for (var mesh in glb.json["meshes"]) {
      for (var prim in mesh["primitives"]) {
        if (prim["attributes"]["POSITION"] == accIdx) {
          isPosition = true;
        }
      }
    }
...
Future<void> optimizeGlb(GlbFile glb, double ratio) async {
  ...
  for (var mesh in (glb.json["meshes"] as List)) {
    for (var prim in (mesh["primitives"] as List)) {
      ...
      final result = QEMDecimator(vData, iData).decimate((iData.length ~/ 3 * ratio).toInt());
      ...
      views.add({"buffer": 0, "byteOffset": vOff, "byteLength": vBytes.length, "target": 34962});
      accessors.add({"bufferView": views.length - 1, "byteOffset": 0, "componentType": 5126, "count": result.v.length ~/ 3, "type": "VEC3", "min": result.min, "max": result.max});
      attr["POSITION"] = accessors.length - 1;

      views.add({"buffer": 0, "byteOffset": iOff, "byteLength": iBytes.length, "target": 34963});
      accessors.add({"bufferView": views.length - 1, "byteOffset": 0, "componentType": isU32 ? 5125 : 5123, "count": result.i.length, "type": "SCALAR"});
...
class QEMDecimator {
  final List<double> v; final List<int> i;
  QEMDecimator(this.v, this.i);
  DecimationResult decimate(int target) => DecimationResult(v, i, [0.0, 0.0, 0.0], [1.0, 1.0, 1.0]); 
}

class DecimationResult {
  final List<double> v; final List<int> i; final List<double> min, max;
...
List<double> _getF32(Map acc, List views, ByteData bin) {
  ...
  return List.generate(count * 3, (i) {
    return bin.getFloat32(off + (i ~/ 3) * stride + (i % 3) * 4, Endian.little);
  });
}
...
List<int> _getIdx(Map acc, List views, ByteData bin) {
  ...
  return List.generate(count, (i) {
    return isU32 ? bin.getUint32(off + i * 4, Endian.little) : bin.getUint16(off + i * 2, Endian.little);
  });
}
